...
let createblog=(req,res)=>{
      ...
      newBlog.save((err,result)=>{
       if(err){
        logger.captureError(`error occured:${err}`,'Blog Controller:Create Blog',10)
        let apiResponse=res.generate(ture,'Blog Is Not Created',500,null)
        res.send(apiResponse)
       } else {
        logger.captureInfo('Blog Is Created Successfully','Blog Controller:create Blog',8)
